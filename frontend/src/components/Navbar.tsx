          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8 shrink-0">
              {navLinks.map((link) => (
                <Link
                  key={link.to}
                  to={link.to}
                  className={`font-modern text-sm font-medium transition-colors hover:text-accent link-underline pb-1 ${location.pathname === link.to
                      ? "text-accent"
                      : "text-gray-300"
                    }`}
                >
            ...
            >
              <div className="w-6 h-5 relative flex flex-col justify-between">
                <span
                  className={`block h-0.5 w-6 bg-white rounded transition-all ${isOpen ? "rotate-45 translate-y-2" : ""
                    }`}
                />
                <span
                  className={`block h-0.5 w-6 bg-white rounded transition-all ${isOpen ? "opacity-0" : ""
                    }`}
                />
                <span
                  className={`block h-0.5 w-6 bg-white rounded transition-all ${isOpen ? "-rotate-45 -translate-y-2" : ""
              ...
              <div className="px-4 py-6 space-y-4">
                {navLinks.map((link) => (
                  <Link
                    ...
                    className={`block py-2 font-modern text-base font-medium transition-colors ${location.pathname === link.to
                        ? "text-accent"
                        : "text-gray-300"
                      }`}
                  >
